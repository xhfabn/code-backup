...
export default function OnboardingPage() {
  ...
  const t = {
    title:
      formData.uiLanguage === "en" ? "Welcome to 面试通" : "欢迎来到 面试通",
    sub:
      formData.uiLanguage === "en"
        ? "Master tech interviews with smart review"
        : "只需几步，开启你的智能复习之旅",
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="space-y-3 group">
            <label className="text-xs font-bold text-gray-500 uppercase tracking-widest ml-1 flex items-center gap-2">
              <User
                size={14}
                className="group-focus-within:text-[#3b82f6] transition-colors"
              />
              {t.labelUser}
            </label>
            <input
              required
              className="w-full px-5 py-4 bg-white/50 border border-transparent rounded-2xl focus:ring-4 focus:ring-blue-100 focus:bg-white focus:border-[#3b82f6]/30 outline-none transition-all placeholder:text-gray-300 shadow-sm text-gray-900 font-medium"
          ...
          <div className="space-y-3 group">
            ...
            >
              <SelectTrigger className="w-full h-[60px] px-5 bg-white/50 border-transparent rounded-2xl focus:ring-4 focus:ring-blue-100 outline-none transition-all shadow-sm font-medium">
